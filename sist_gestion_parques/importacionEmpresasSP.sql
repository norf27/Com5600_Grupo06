...
CREATE OR ALTER PROCEDURE Concesiones.SP_Importar_EmpresasCSV
...
BEGIN
    ...
    TRUNCATE TABLE Concesiones.STG_Empresa;
    ...
    END TRY
    BEGIN CATCH
        INSERT INTO Concesiones.Log_Errores_Importacion (Archivo_Origen, Fila_Contenido_Raw, Motivo_Error)
        VALUES (@RutaArchivo, 'SISTEMA_CRÍTICO', 'Error físico al abrir o leer el archivo CSV: ' + ERROR_MESSAGE());
        RETURN;
    END CATCH;

    -- Limpieza básica de los datos en Staging (reemplazo de NA por nulos y recortes)
    UPDATE Concesiones.STG_Empresa
    ...
    -- 3. Validar errores (ejemplo: organizaciones sin nombre o nombres que exceden el VARCHAR(100) de la tabla original)
    INSERT INTO Concesiones.Log_Errores_Importacion (Archivo_Origen, Fila_Contenido_Raw, Motivo_Error)
    SELECT 
        @RutaArchivo,
    ...
    INNER JOIN Concesiones.STG_Empresa S ON UPPER(TRIM(E.Nombre)) = UPPER(TRIM(S.organizacion))
    ...
    SELECT DISTINCT
        ...
        LEFT(S.telefono, 20),
        'A'
    FROM Concesiones.STG_Empresa S
    WHERE S.organizacion IS NOT NULL 
      AND S.organizacion <> ''
      AND LEN(S.organizacion) <= 100
      AND NOT EXISTS (
